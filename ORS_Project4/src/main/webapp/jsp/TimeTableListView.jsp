...
<body>
	...
	%>
			...
			<table width="100%">
				...
				<tr>
					<td align="right"><label>Course Name :</label> <%=HTMLUtility.getList("clist", String.valueOf(bean.getCourseId()), cList) %></td>
	
		  	<td align="left"><lable>Semester</lable><input type="text" name="Semester" value=<%=ServletUtility.getParameter("Semester", request) %>>
		  	
		  	<input type="submit" name="operation"value="<%=TimeTableListCtl.OP_SEARCH%>">
		  	
		  	 <input type="submit" name="operation"value="<%=TimeTableListCtl.OP_RESET %>"></td></td>
			
						<!--  <label>Subject Name :</label> <%=HTMLUtility.getList("slist", String.valueOf(bean.getSubjectId()), sList) %>-->

						      		<%-- 	</label>Exam Date<input type="date" name="Exdate"  readonly="readonly"
