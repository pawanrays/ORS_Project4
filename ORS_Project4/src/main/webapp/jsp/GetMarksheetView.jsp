	...
	%>
				...
				<table border="1" width="100%">
					...
					<tr align="center" style="background: darkcream">
						...
						<td>100</td>
						<td>33</td>
						<td><%=phy %> <%if(phy<33){%> <span style="color: red">*</span>
							<% } %></td>

						<td align="center" style="background: darkcream">
							<%
					if(phy <=100 && phy >85){ %> A+ <%} else if(phy<=85 && phy > 75 ) {%>B+
							<%} else if(phy<=75 && phy > 65 ) {%>B <%} else if(phy<=65 && phy > 55 ){ %>C+
							<%} else if(phy<=55 && phy >=33  ){ %>C <%} else if(phy< 33 && phy >= 0 ) {%><span
					...
					<tr align="center" style="background: darkcream">
						...
						<td>100</td>
						<td>33</td>
						<td><%=chem %> <%if(chem<33){%> <span style="color: red">*</span>
							<% } %></td>

						<td align="center" style="background: darkcream">
							<%
					if(chem <=100 && chem >85){ %> A+ <%} else if(chem<=85 && chem > 75 ) {%>B+
							<%} else if(chem <=75 && chem > 65 ) {%>B <%} else if(chem <=65 && chem > 55 ){ %>C+
							<%} else if(chem <=55 && chem >=33  ){ %>C <%} else if(chem < 33 && chem >= 0 ) {%><span
							style="color: red"> Fail</span> <% } %>
						</td>
					</tr>
					...
					<tr align="center" style="background: darkcream">
						...
						<td>100</td>
						<td>33</td>
						<td><%=math %> <%if(math<33){%> <span style="color: red">*</span>
							<% } %></td>

						<td align="center" style="background: darkcream">
							<%
					if(math <=100 && math >85){ %> A+ <%} else if(math <=85 && math > 75 ) {%>B+
							<%} else if( math <=75 && math > 65 ) {%>B <%} else if(math <=65 && math > 55 ){ %>C+
							<%} else if(math <=55 && math >=33  ){ %>C <%} else if(math < 33 && math >= 0 ) {%><span
							style="color: red"> Fail</span> <% } %>
						</td>
					</tr>
				</table>
				...
				<table border="1" width="100%">
					...
					<tr>
						<th style="background: darkcream"><%=total %> <% if(total<99 || phy<33|| chem<33|| 	math<33){ %>
							<span style="color: red">*</span> <% } %></th>

						<th style="background: darkcream"><%=perc %> %</th>
						<th>
							<% if(perc <100 && perc >= 60){ %>1<sup>st</sup> <%} else if(perc <60 && perc >=40){ %>2<sup>nd</sup>
							<%} else if(perc <40 && perc >=0){ %>3<sup>rd</sup> <%} %>
						</th>

						<th style="background: darkcream">
							<%
				if(phy >=33 && chem>=33 && math >= 33) { %> <span
							style="color: green"> Pass</span> <%}else{ %> <span
							style="color: red"> Fail</span> <% } %>
